        ...
        sortedRounds.forEach(roundNum => {
            ...
            if (roundsWithBonuses.has(roundNum)) {
                ...
                if (editBonusMode.has(roundNum)) {
                    // Edit mode: show input fields (one per bonus)
                    let inputsHtml = '';
                    if (roundBonuses.length > 0) {
                        roundBonuses.forEach((bonus, index) => {
                            const value = bonus.value.replace('x2', '0');
                            const numValue = parseInt(value);
                            const bonusValue = !isNaN(numValue) ? numValue : 0;
                            const icon = bonus.type === 'handweis' ? '👋' : '🏓';
                            inputsHtml += `<div class="bonus-input-row">${icon} <input type="number" class="edit-input" data-round="${roundNum}" data-player="${player}" data-type="bonus" data-index="${index}" value="${bonusValue}" /></div>`;
                        });
                    } else {
                        // If no bonuses yet, show one empty input
                        inputsHtml = `<div class="bonus-input-row"><input type="number" class="edit-input" data-round="${roundNum}" data-player="${player}" data-type="bonus" data-index="0" value="" /></div>`;
                    }
                    cellContent = inputsHtml;
                } else {
                    // Display mode
                    let bonusText = '';
                    if (roundBonuses.length > 0) {
...
function saveBonusColumn(roundNumber) {
    ...
    const bonusUpdates = {}; // Track bonuses by player

    // Group inputs by player
    inputs.forEach(input => {
        const player = input.dataset.player;
        if (!bonusUpdates[player]) {
            bonusUpdates[player] = [];
        }
        const value = input.value.trim() === '' ? null : parseInt(input.value);
        if (!isNaN(value) && value !== null) {
            bonusUpdates[player].push(value);
        }
    });

    // Remove old bonuses for this round
    game.bonuses = game.bonuses.filter(b => b.roundNumber !== roundNumber);

    // Add new bonuses from inputs
    for (const player in bonusUpdates) {
        bonusUpdates[player].forEach(value => {
            if (value !== 0) {
